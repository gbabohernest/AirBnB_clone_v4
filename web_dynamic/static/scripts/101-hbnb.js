...
$(document).ready(function () {
  ...
  // Listen for click on the span next to Review h2
  $('h2#Reviews + span').click(function () {
    const reviewsToggle = $(this);

    // check if review should be fetched or hidden
    if (reviewsToggle.text() === 'Show') {
  ...
  function fetchReview() {
    // fetch, parse, and display reviews
    $.get('https://localhost:5001/api/v1/reviews/', function (reviews) {
      for (const review of reviews) {
        const reviewElement = `<div class="review">
            <h3>${review.user}</h3>
            <p>${review.text}</p>
            </div>`;
        $('div.reviews').append(reviewElement);
      }
    });
  }
});
